<template>
    <!-- image preview·support svg -->
     <!-- svg container -->
    <div
        v-if="isSvg"
        class="image-preview"
        v-html="svgContentStr"
        :style="{ width: width, height: height }"
        ref="svgParentContainer"
    />
     <!-- image container -->
    <img
        v-else
        class="image-preview"
        :src="imageLink"
        :alt="alt"
        @error="isValid = false"
        @load="loading = false"
        loading="lazy"
        @loadstart="loading = true"
        :style="{ width: width, height: height }"
    />
</template>

<script lang="ts" setup>
import { defineProps, computed, ref, watch, nextTick } from 'vue';
import axios from 'axios';

const props = defineProps({
    width: { // 图片宽度
        type: String,
        default: '100%'
    },
    height: { // 图片高度
        type: String,
        default: '100%'
    },
    src: { // 图片链接
        type: String,
        required: true
    },
    fallback: { // 加载失败容错地址
        type: String,
        // eslint-disable-next-line max-len
        default: 'data:image/png;base64,'
    },
    alt: { // img.alt属性
        type: String,
        required: false
    }
});

const loading = ref<boolean>(false); // 图片加载loading
const isValid = ref<boolean>(Boolean(props.src)); // 链接是否有效
const imageLink = computed<string>(() => (isValid.value ? props.src : props.fallback)); // 真实显示出来的图片地址
const isSvg = computed<boolean>(() => /(\.svg)$/igm.test(imageLink.value)); // 是否是svg格式的图片
const svgContentStr = ref<string>(''); // svg字符串内容
const svgParentContainer = ref<HTMLElement>(document.body); // svg元素的父元素

const fetchSvg = (address: string): Promise<void> => { // 请求svg图像
    loading.value = true;
    return axios.get(address).then(({ data }) => {
        [
            svgContentStr.value, // 保存svg内容
            isValid.value // 更改图片链接有效状态
        ] = [
            data,
            true
        ];
        nextTick(() => {
            const svgEl = svgParentContainer.value.querySelector('svg');
            if (svgEl !== null) {
                const svgRect: DOMRect = svgEl.getBBox();
                svgEl.setAttribute('viewBox', `${svgRect.x} ${svgRect.y} ${svgRect.width} ${svgRect.height}`); // 让svg自适应展示
            }
        });
    }).catch(() => {
        [
            svgContentStr.value, // 保存svg内容
            isValid.value // 更改图片链接有效状态
        ] = [
            '',
            false
        ];
    }).finally(() => {
        loading.value = false;
    });
};

watch(() => props.src, (n, o) => {
    isValid.value = n !== o; // 当src链接发生变化时，重置链接有效状态
});

watch(imageLink, (n, o) => {
    if (n !== o && isSvg.value) { // 如果是svg就加载svg图片
        fetchSvg(n);
    }
}, { immediate: true });

</script>

<style scoped lang="scss">
.image-preview {
  overflow: visible;
  background-color: transparent;
  margin: 0;
  padding: 0;
  border: 0;
  outline: 0;
  vertical-align: inherit;

  :deep(svg) {
    width: 100%;
    height: 100%;
    overflow: hidden;
  }
}
</style>
