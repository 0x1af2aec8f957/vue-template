...
declare global { // 声明进入全局命名空间的类型，或者增加全局命名空间中的现有声明
    ...
    /**
     * 将数组转换为对象（数组子项的特定 key 转换为对象的key）
     ...
     */
    type ArrayInnerPair<T extends Array<any>, U extends ObjectPropertyKey<T[number]> = ObjectPropertyKey<T[number]>, P extends ObjectPropertyKey<T[number]> = ObjectPropertyKey<T[number]>> = Record<ObjectPropertyValue<T[number], U>, ObjectPropertyValue<T[number], P>> 
